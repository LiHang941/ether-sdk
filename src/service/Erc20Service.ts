...
export class Erc20Service extends BaseService {
  ...
  async batchGetTokenInfo(
    ...
    const [...resultList] = await this.connectInfo.multiCall().callObj(
  ...
  async batchGetBalance(
    ...
    result.forEach((it) => {
      data[it.address] = {
        address: it.address,
        amount: new BigNumber(it.balance || '0').div(10 ** Number.parseInt(it.decimals as string || '0', 10)).toFixed(),
        value: it.balance || '0',
        decimal: Number.parseInt(it.decimals as string|| '0', 10),
      }
    })
    return data
  }
  ...
  async batchGetBalanceInfo(
    ...
    result.forEach((it, index) => {
      data[it.address] = new Balance(tokens[index], user, new BigNumber(it.balance).div(10 ** parseInt(it.decimals||"0")).toFixed())
    })
    return data
  }
  ...
  async batchGetBalanceAndAllowance(
    ...
    result.forEach((it, index) => {
      data[it.address] = new BalanceAndAllowance(tokens[index], user, new BigNumber(it.balance).div(10 ** parseInt(it.decimals || "0",10)).toFixed(), new BigNumber(it.allowance as string).div(10 ** parseInt(it.decimals||"0")).toFixed(), spender)
    })
    return data
  }
}
